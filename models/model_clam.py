...
import numpy as np
import os
from models.topk import SmoothTop1SVM
...
if __name__ == "__main__":
    bag_size = 100
    feat_dim = 1024
    x = torch.randn((bag_size, feat_dim)).cuda()
    net = CLAM_MB(instance_eval=True).cuda()
    logits, Y_prob, Y_hat, results_dict = net(h=x, label=torch.tensor([1]))
    print(f"logits: {logits.size()} Y_prob: {Y_prob} Y_hatL {Y_hat}")
    print(results_dict)